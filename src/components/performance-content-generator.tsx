      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
          <div className="flex justify-center">
            <TabsList className="grid w-full max-w-2xl grid-cols-4 h-14 p-1 bg-card shadow-lg border-0">
              <TabsTrigger value="generator" className="flex items-center gap-2 text-sm font-medium text-muted-foreground data-[state=active]:bg-gradient-to-r data-[state=active]:from-amber-500 data-[state=active]:to-orange-500 data-[state=active]:text-white hover:bg-muted rounded-lg transition-all duration-200">
                <Sparkles className="h-4 w-4" />
                Generator
              </TabsTrigger>
              <TabsTrigger value="progress" disabled={!currentJob} className="flex items-center gap-2 text-sm font-medium text-muted-foreground data-[state=active]:bg-gradient-to-r data-[state=active]:from-amber-500 data-[state=active]:to-orange-500 data-[state=active]:text-white hover:bg-muted disabled:text-muted-foreground rounded-lg transition-all duration-200">
                <RefreshCw className="h-4 w-4" />
                Progress
              </TabsTrigger>
              <TabsTrigger value="results" disabled={jobDrafts.length === 0} className="flex items-center gap-2 text-sm font-medium text-muted-foreground data-[state=active]:bg-gradient-to-r data-[state=active]:from-amber-500 data-[state=active]:to-orange-500 data-[state=active]:text-white hover:bg-muted disabled:text-muted-foreground rounded-lg transition-all duration-200">
                <BarChart3 className="h-4 w-4" />
                Results
              </TabsTrigger>
              <TabsTrigger value="analytics" className="flex items-center gap-2 text-sm font-medium text-muted-foreground data-[state=active]:bg-gradient-to-r data-[state=active]:from-amber-500 data-[state=active]:to-orange-500 data-[state=active]:text-white hover:bg-muted rounded-lg transition-all duration-200">
              ...
              <Card className="shadow-xl border border-border bg-card/90 backdrop-blur-sm">
                ...
                <CardContent className="space-y-6">
                  <div className="space-y-3">
                    <Label htmlFor="intent" className="text-sm font-semibold text-foreground">Strategic Intent</Label>
                    <Select value={selectedIntent} onValueChange={setSelectedIntent}>
                      <SelectTrigger className="h-12 border-border bg-muted hover:bg-muted/80 transition-colors text-foreground">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="bg-card border-border">
                        {CONTENT_INTENTS.map((intent) => (
                          <SelectItem key={intent.value} value={intent.value} className="p-4 text-foreground hover:bg-muted">
                            <div>
                              <div className="font-medium text-foreground">{intent.label}</div>
                              <div className="text-sm text-muted-foreground mt-1">{intent.description}</div>
                            </div>
                          </SelectItem>
                        ))}
                      </SelectContent>
                  ...
                  <div className="space-y-3">
                    <Label htmlFor="topic" className="text-sm font-semibold text-foreground">Executive Topic or Strategic Message</Label>
                    <Textarea
                      id="topic"
                      value={topic}
                      onChange={(e) => setTopic(e.target.value)}
                      className="min-h-[140px] border-border bg-muted hover:bg-muted/80 transition-colors text-foreground placeholder:text-muted-foreground focus:border-amber-500 focus:ring-amber-500/20"
              ...
              <Card className="shadow-xl border border-border bg-card/90 backdrop-blur-sm">
                ...
                <CardContent className="space-y-6">
                  <div className="grid gap-4">
                    ...
                    <div className="flex justify-between items-center p-4 bg-muted rounded-lg">
                      <span className="font-semibold text-foreground">Strategic Intent:</span>
                      ...
                      <Card 
                        ...
                        className={`cursor-pointer transition-all duration-300 hover:shadow-lg ${
                          isSelected 
                            ...
                            : 'border border-border hover:border-border/80 bg-card hover:bg-muted'
                        }`}
                        ...
                        <CardContent className="p-6">
                          <div className="flex items-start gap-4">
                            <div className={`p-3 rounded-xl transition-all duration-300 ${
                              isSelected ? 'bg-gradient-to-r from-amber-500 to-orange-500' : 'bg-muted'
                            }`}>
                            ...
                            <div className="flex-1">
                              <div className="flex items-center justify-between mb-3">
                                <h3 className="font-bold text-lg text-foreground">{variant.name}</h3>
                                {isSelected && (
        ...
        <TabsContent value="progress" className="space-y-8">
          {currentJob ? (
            <Card className="shadow-xl border border-border bg-card/90 backdrop-blur-sm">
              <CardHeader className="pb-8">
                <CardTitle className="flex items-center justify-between">
                  <span className="flex items-center gap-3 text-2xl text-foreground">
              ...
              <CardContent className="space-y-8">
                <div className="space-y-4">
                  ...
                  <Progress value={currentJob.progress} className="w-full h-3 bg-muted" />
                </div>
            ...
            <div className="text-center py-20">
              <div className="p-4 bg-muted rounded-full w-16 h-16 mx-auto mb-6 flex items-center justify-center">
                <Clock className="h-8 w-8 text-muted-foreground" />
              </div>
              <h3 className="text-xl font-semibold text-foreground mb-2">No Active Generation</h3>
              <p className="text-muted-foreground text-lg">Start generating content to see progress here.</p>
            </div>
          )}
        </TabsContent>
          ...
          {jobDrafts.length > 0 ? (
            <>
              <div className="text-center mb-12">
                <h3 className="text-3xl font-bold bg-gradient-to-r from-foreground to-muted-foreground bg-clip-text text-transparent mb-4">
              ...
              <div className="grid gap-8">
                {jobDrafts.map((draft, index) => {
                  ...
                  return (
                    <Card key={draft.id} className="overflow-hidden shadow-xl border border-border bg-card/90 backdrop-blur-sm">
                        ...
                        {variant && (
                          <CardDescription>
                            <div className="flex items-center gap-4 text-xs text-muted-foreground">
                              <span><strong className="text-muted-foreground">Focus:</strong> {variant.focus}</span>
                              <span><strong className="text-muted-foreground">Expected:</strong> {variant.expectedOutcome}</span>
                              <span><strong className="text-muted-foreground">Generation time:</strong> {draft.metadata.generation_time_ms}ms</span>
                            </div>
                          </CardDescription>
                        )}
                      </CardHeader>
                      ...
                      <CardContent className="space-y-6 pt-6">
                        <div className="prose prose-base max-w-none">
                          <Textarea
                            ...
                            className="bg-muted border-border text-foreground leading-relaxed text-base shadow-sm focus:border-amber-500 focus:ring-amber-500/20 resize-none overflow-hidden"
                        ...
                        {draft.content.hashtags && draft.content.hashtags.length > 0 && (
                          <div className="space-y-3">
                            <h5 className="font-semibold text-foreground">Recommended Hashtags:</h5>
                            <div className="flex flex-wrap gap-3">
                              {draft.content.hashtags.map((tag, tagIndex) => (
                                <Badge key={tagIndex} className="bg-muted hover:bg-muted/80 text-muted-foreground px-3 py-1 text-sm font-medium transition-colors">
                                  {tag}
                                </Badge>
                              ))}
                            </div>
            ...
            <div className="text-center py-20">
              ...
              <h3 className="text-xl font-semibold text-foreground mb-2">No Content Generated</h3>
              <p className="text-muted-foreground text-lg">Use the generator to create strategic variants and see results here.</p>
            </div>
          )}
        </TabsContent>
          ...
          <Card className="shadow-xl border border-border bg-card/90 backdrop-blur-sm">
            ...
            <CardContent>
              <div className="text-center py-20">
                ...
                <h3 className="text-2xl font-bold text-foreground mb-4">Analytics Coming Soon</h3>
                <p className="text-lg text-muted-foreground mb-2">Performance analytics will appear here after content is posted and tracked.</p>
                <p className="text-muted-foreground">Real-time engagement tracking and voice evolution insights in development.</p>
