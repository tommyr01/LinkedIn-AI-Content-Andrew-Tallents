...
class StrategicIntegrationTester {
  ...
  private async testHistoricalAnalysis(): Promise<any> {
    ...
    return {
      ...
      relatedPostsCount: insights.related_posts ? insights.related_posts.length : 0,
      topPerformersCount: insights.top_performers ? insights.top_performers.length : 0,
      avgEngagementScore: insights.performance_context.avg_engagement
    }
  }
  ...
  private async testStrategicIntelligence(): Promise<any> {
    ...
    const strategicInsights = await performanceInsightsService.generateStrategicInsights(
      ...
      insights.performance_context.avg_engagement
    )

    return {
      historical: {
        confidenceLevel: insights.confidence_level,
        relatedPosts: insights.related_posts ? insights.related_posts.length : 0,
        topPerformers: insights.top_performers ? insights.top_performers.length : 0,
